...
import numpy as np
from echo.util.tensors import batch_broadcast
import torch

from echo.util.registry import Registry
...
class echo_SDE(SDE):
    ...
    def copy(self):
        return echo_SDE(self.beta_min, self.beta_max, N=self.N)
    ...
    def reverse_sampling(self,x0,xt,t):
        ...
        if timestep > 0:
            posterior_variance = (
                betas[timestep] * (1.0 - alphas_cumprod[timestep-1]) / (1.0 - alphas_cumprod[timestep])
            )
