...
from echo import sampling
from echo.sdes import SDERegistry
from echo.backbones import BackboneRegistry
from echo.util.inference import evaluate_model
from echo.util.other import pad_spec
...
class ScoreModel(pl.LightningModule):
    ...
    def _step(self, batch, batch_idx):
        ...
        n = y-x
        n = n / torch.max(torch.abs(n))
        self.n_set.append(n)
        n_decouple=self.n_set[np.random.randint(0,len(self.n_set))]
        if n_decouple.shape[0] > t.shape[0]:
            n_decouple = n_decouple[:t.shape[0]]
        else:
            n_decouple = torch.cat([n_decouple]*(t.shape[0]//n_decouple.shape[0]+1),dim=0)[:t.shape[0]]
        beta = torch.rand(t.shape[0], device=x.device)
        n_remixup = t[:, None, None, None]*(beta*n + (1-beta)*n_decouple)
        z = z + n_remixup
